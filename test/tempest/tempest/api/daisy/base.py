...
class BaseDaisyTest(tempest.test.BaseTestCase):
    ...
    def get_discover_host_detail(self, host_meta):
        host_detail =
            self.daisy_client.hosts.get_discover_host_detail(host_meta)
        return host_detail
    ...
    def _clean_all_discover_host(self):
        host_meta = {}
        hosts_list_generator =
            self.daisy_client.hosts.list_discover_host(**host_meta)
        hosts_list = [hosts for hosts in hosts_list_generator]
        if hosts_list:
            for host in hosts_list:
    ...
    def _clean_all_config_set(self):
        ...
        config_set_list =
            [config_set for config_set in config_set_list_generator]
        if config_set_list:
    ...
    def _clean_all_config_file(self):
        ...
        config_file_list =
            [config_file for config_file in config_file_list_generator]
        if config_file_list:
    ...
    def update_cinder_volume(self, cinder_volume_id, **cinder_volume_meta):
        cinder_volume_info =
            self.daisy_client.disk_array.cinder_volume_update(
                cinder_volume_id, **cinder_volume_meta)
        return cinder_volume_info
    ...
    def get_cinder_volume_detail(self, cinder_volume_id):
        cinder_volume_info =
            self.daisy_client.disk_array.cinder_volume_detail(
                cinder_volume_id)
        return cinder_volume_info
    ...
    def get_service_disk_detail(self, service_disk_id):
        service_disk_detail =
            self.daisy_client.disk_array.service_disk_detail(service_disk_id)
    ...
    def _clean_all_physical_node(self):
        ...
        physical_node_list =
            [physical_node for physical_node in physical_node_list_generator]
