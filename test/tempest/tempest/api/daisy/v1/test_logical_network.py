...
class TecsLogicalNetworkTest(base.BaseDaisyTest):
    ...
    def _verify_logical_params(self, cluster_meta, fake_logical):
        ...
        # for logic_network in cluster_meta.get('logic_networks', []):
        #     logic_network['subnets'] =  \
        #         [dict(filter(lambda paris: paris[0] in TecsLogicalNetworkTest.SUBNET_FILTER, subnet.items()))
        #          for subnet in logic_network.get('subnets', [])]
        #
        #     for subnet in logic_network.get('subnets', []):
        #         subnet.get('dns_nameservers', []).sort()
        #         sorted(subnet.get('floating_ranges', []), key=lambda x : x[0])
    ...
    # STC-A-Daisy_Logical_Network-0005
    # def test_network_params_valid_check_exc(self):
        # fake_cluster = self.fake.fake_cluster_parameters()
        # fake_network = self.fake.fake_network_parameters()

        # tmp_network1 = copy.deepcopy(fake_network)
        # tmp_network1['segmentation_type'] += ',test'
        # fake_cluster.update({'networking_parameters': tmp_network1})
        # self.assertRaisesMessage(
            # client_exc.HTTPBadRequest,
            # "400 Bad Request: Segmentation_type of networking_parameters is not valid. (HTTP 400)",
            # self.add_cluster, **fake_cluster)

        # tmp_network2 = copy.deepcopy(fake_network)
        # tmp_network2['vlan_range'].append(10000)
        # fake_cluster.update({'networking_parameters': tmp_network2})
        # '''
        # self.assertRaisesMessage(
            # client_exc.HTTPBadRequest,
            # "400 Bad Request: Range params must be pair. (HTTP 400)",
            # self.add_cluster, **fake_cluster)
        # '''
        # ex = self.assertRaises(client_exc.HTTPForbidden, self.add_cluster, **fake_cluster)
        # self.assertIn("range must be existed in pairs", str(ex))
        # TODO:vni_range,gre_range

    # STC-A-Daisy_Logical_Network-0006
    # def test_logical_params_valid_check_exc(self):
        # private_network = self.private_network_add()
        # fake_cluster = self.fake.fake_cluster_parameters(private_network)
        # fake_logical = self.fake.fake_logical_parameters(private_network)
        # fake_network = self.fake.fake_network_parameters()
        ...
        # for logical in tmp_fake_logical1:
            # logical['name'] = "test"
        ...
        # self.assertRaisesMessage(
            # client_exc.HTTPBadRequest,
            # "400 Bad Request: Logic network name segment is repetition. (HTTP 400)",
            # self.add_cluster, **fake_cluster)

        # err = "400 Bad Request: Segmentation_id is out of private network %s of %s.Vaild range is [%s, %s]. (HTTP 400)"
        # tmp_fake_logical2 = copy.deepcopy(fake_logical)
        # tmp_fake_logical2[0]['segmentation_type'] += "test"
        # fake_cluster.update({'logic_networks': tmp_fake_logical2})
        # self.assertRaisesMessage(
            # client_exc.HTTPBadRequest,
            # "400 Bad Request: Segment segmentation_type is out of valid range. (HTTP 400)",
            # self.add_cluster, **fake_cluster)

        # tmp_fake_logical3 = copy.deepcopy(fake_logical)
        # tmp_fake_logical3[0]['physnet_name'] += "test"
        # fake_cluster.update({'logic_networks': tmp_fake_logical3})
        # self.assertRaisesMessage(
            # client_exc.HTTPBadRequest,
            # "400 Bad Request: Segment physnet_name is out of valid range. (HTTP 400)",
            # self.add_cluster, **fake_cluster)

        # tmp_fake_logical4 = copy.deepcopy(fake_logical)
        # tmp_fake_logical4[0]['segmentation_id'] = 10000
        # fake_cluster.update({'logic_networks': tmp_fake_logical4})
        # self.assertRaisesMessage(
            # client_exc.HTTPBadRequest,
            # err % ("vlan_range", tmp_fake_logical2[0]['physnet_name'],
                   # private_network.vlan_start, private_network.vlan_end),
            # self.add_cluster, **fake_cluster)

        # tmp_fake_logical6 = copy.deepcopy(fake_logical)
        # tmp_fake_logical6[0]['segmentation_id'] = 10000
        # tmp_fake_logical6[0]['segmentation_type'] = "vxlan"
        # fake_cluster.update({'logic_networks': tmp_fake_logical6})

        # self.assertRaisesMessage(
            # client_exc.HTTPBadRequest,
            # "400 Bad Request: Segmentation_id is out of vni_range. (HTTP 400)",
            # self.add_cluster, **fake_cluster)

        # tmp_fake_logical6 = copy.deepcopy(fake_logical)
        # tmp_fake_logical6[0]['segmentation_id'] = 10000
        # tmp_fake_logical6[0]['segmentation_type'] = "gre"
        # fake_cluster.update({'logic_networks': tmp_fake_logical6})
        # self.assertRaisesMessage(
            # client_exc.HTTPBadRequest,
            # "400 Bad Request: Segmentation_id is out of gre_id_range. (HTTP 400)",
            # self.add_cluster, **fake_cluster)
