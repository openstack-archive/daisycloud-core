...
class BaseTestCase(testtools.testcase.WithAttributes,
    ...
    def sdn_skip_check(cls):
        ...
        if BaseTestCase.is_sdn:
            raise testtools.TestCase.skipException("skip : SDN " + " not support this function")            
    ...
    def dvs_skip_check(cls):
        ...
        if BaseTestCase.is_dvs:
            raise testtools.TestCase.skipException("skip : DVS " + " not support this function")            
    ...
    def lvm_skip_check(cls):
        ...
        if BaseTestCase.is_lvm:
            raise testtools.TestCase.skipException("skip : LVM " + " not support this function")            
       
    #zfl skip connect to vm
    @classmethod
    def connectvm_skip_check(cls):
        if not hasattr(cls, 'can_connect_vm'):
            BaseTestCase.can_connect_vm = (CONF.network.tenant_networks_reachable or CONF.compute.use_floatingip_for_ssh)

        if not BaseTestCase.can_connect_vm:
            raise testtools.TestCase.skipException("skip : no network reachable to ssh vm")
