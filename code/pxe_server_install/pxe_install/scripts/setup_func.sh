...
function custom_ks_hugepages
{
    ...
    if [[ $pages != "" ]]; then
        ...
        bootloader_append_holder="intel_iommu=on iommu=pt transparent_hugepage=never default_hugepagesz=${sizes} hugepagesz=${sizes} hugepages=${pages}"
...
function custom_ks_isolcpus
{
    ...
    if [[ $isolcpus != "" ]]; then
        if [[ $bootloader_append_holder != "" ]];then
            bootloader_append_holder_isolcpus=" isolcpus=${isolcpus}"
            bootloader_append_holder=$bootloader_append_holder$bootloader_append_holder_isolcpus
        else
            bootloader_append_holder="isolcpus=${isolcpus}"
        fi
    fi
