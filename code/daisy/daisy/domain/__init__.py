...
CONF = cfg.CONF
CONF.import_opt('task_executor', 'daisy.common.config', group='task')
...
def _import_delayed_delete():
    # glance_store (indirectly) imports daisy.domain therefore we can't put
...
class TaskExecutorFactory(object):
    ...
    def new_task_executor(self, context):
        try:
            ...
            executor_cls = ('daisy.async.%s_executor.'
