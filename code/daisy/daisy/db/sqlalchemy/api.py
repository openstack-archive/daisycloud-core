...
def _is_pre_install(session, host_ref):
    role_filter = 'host_id="%s" and deleted=0' % host_ref.id
    host_roles = _query_table(session, models.HostRole, role_filter)
    flag = True
    for host_role in host_roles:
        if host_role.status not in ['init', 'install-failed']:
            flag = False
            break
    return flag
    ...
    with session.begin():
        ...
        if host_id:
            ...
            if values.has_key('interfaces'):
                ...
                if host_ref.discover_mode == 'SSH' and values.get(
                        'cluster') and _is_pre_install(session, host_ref):
