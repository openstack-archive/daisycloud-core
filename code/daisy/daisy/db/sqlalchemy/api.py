...
def cidr_convert_ip_ranges(cidr):
    ...
    if ip_addr_min.split('.')[3]=='0':
        ip_addr_min=ip_addr_min.split('.')[0]+'.'+ip_addr_min.split('.')[1]+'.'+ip_addr_min.split('.')[2]+'.1'
    return [ip_addr_min,ip_addr_max]
    
def get_ip_with_equal_cidr(cluster_id,network_plane_name,session):
    equal_cidr_network_plane_id_list=[]
    available_ip_list=[]

    sql_network_plane_cidr="select networks.cidr from networks where networks.name='"+network_plane_name+"' and networks.cluster_id='"+cluster_id+"' and networks.deleted=0"
    query_network_plane_cidr = session.execute(sql_network_plane_cidr).fetchone()
    ...
    sql_all_network_plane_info="select networks.id,networks.cidr from networks where networks.cluster_id='"+cluster_id+"' and networks.deleted=0"
    query_all_network_plane_info = session.execute(sql_all_network_plane_info).fetchall()
    for network_plane_tmp in query_all_network_plane_info:
        ...
        if str_network_cidr==str_query_network_plane_cidr:
            equal_cidr_network_plane_id_list.append(query_network_plane_tmp_info[0])
...
def merge_networks_for_unifiers(cluster_id, assigned_networks):
    ...
    for network_plane in assigned_networks:
        ...
        sql_network_plane_info="select networks.vlan_id,networks.cidr from networks where networks.name='"+network_plane_name+"' and networks.cluster_id='"+cluster_id+"' and networks.deleted=0"
        query_network_plane_info = session.execute(sql_network_plane_info).fetchone()
    ...
    for networks in merged_by_cidr_vlan.values():
        ...
        merged_networks.append({'name':','.join(networks_name),'ip':networks_ip})
    
    return merged_networks
    
def check_ip_exist_and_in_cidr_range(cluster_id,network_plane_name,network_plane_ip,occupied_network_ips,session):
    equal_cidr_network_plane_id=[]
    
    check_ip_if_valid=_ip_checker_re(network_plane_ip)
    ...
    if not check_ip_if_in_cidr:
        msg = "Error:The ip %s is not in cidr %s range!"%(network_plane_ip,network_cidr)
        raise exception.Forbidden(msg)

    available_ip_list=get_ip_with_equal_cidr(cluster_id,network_plane_name,session)
    # allow different networks with same ip in the same interface
    if (network_plane_ip in available_ip_list and
        network_plane_ip not in occupied_network_ips):
        msg = "Error:The IP %s is already exist."%network_plane_ip
        LOG.error(msg)
        raise exception.Forbidden(msg)
    ...
    while True:
        ...
        if ip_tmp not in available_ip_list:
            if inter_tmp > ip_ranges_end_inter:
                msg = "warning:The IP address assigned by IP ranges is already insufficient."
                LOG.warn(msg)
                break
            else:
                return [True,ip_tmp]
...
def change_host_name(values, mangement_ip):
    if mangement_ip:
        values['name'] = "host-" + mangement_ip.replace('.','-')
            
def according_to_cidr_distribution_ip(cluster_id,network_plane_name,session):
    ip_ranges_cidr=[]
    distribution_ip=""

    
    sql_network_plane_info="select networks.id,networks.cidr,networks.network_type from networks where networks.name='"+network_plane_name+"' and networks.cluster_id='"+cluster_id+"' and networks.deleted=0"
    query_network_plane_info = session.execute(sql_network_plane_info).fetchone()
    ...
    if network_type not in ['PRIVATE','EXTERNAL','VXLAN']:
        available_ip_list=get_ip_with_equal_cidr(cluster_id,network_plane_name,session)
        ...
        if query_ip_ranges:
            for ip_ranges_one in query_ip_ranges:
                check_ip_exist_list=check_ip_ranges(ip_ranges_one,available_ip_list)
...
def add_assigned_networks_data(context,network,cluster_id,host_interface_ref,network_plane_names,network_plane_ip,session):
    for network_plane_name in network_plane_names:
        sql_network_plane_id="select networks.id,networks.network_type from networks where networks.name='"+network_plane_name+"' and networks.cluster_id='"+cluster_id+"' and networks.deleted=0"
        query_network_plane_id = session.execute(sql_network_plane_id).fetchone()
        ...
        if network_type == 'VXLAN' or network_type == 'PRIVATE':
            assigned_network['vswitch_type'] = network.get('vswitch_type', 'ovs')
...
def _according_interface_to_add_network_alias(context,interface_assigned_networks,values):
    network_cidrs = []
    session = get_session()
    network_query = session.query(models.Network).filter_by(deleted=False).filter_by(cluster_id=values['cluster']).all()
    ...
    if len(set(network_cidrs)) == 1 and len(network_cidrs) > 1:
        for sub_network_query in network_query:
            if sub_network_query.name in interface_assigned_networks:
                ...
                query_network = session.query(models.Network).filter_by(deleted=False).filter_by(id=sub_network_query.id)
        ...
        if host_id:
            ...
            if values.has_key('interfaces'):
                ...
                for host_interface_info in orig_keys:
                    if host_interface_info.has_key('assigned_networks'):
                        _according_interface_to_add_network_alias(context, host_interface_info['assigned_networks'],values)
                for network in orig_keys:
                    ...
                    if host_interfaces_values.has_key('is_deployment'):
                        if host_interfaces_values['is_deployment']=="True" or host_interfaces_values['is_deployment'] == True or host_interfaces_values['is_deployment'] == "true":
                    ...
                    if values.has_key('cluster'):
                        if network.has_key('assigned_networks'):
                            ...
                            merged_assigned_networks = merge_networks_for_unifiers(values['cluster'],
                                                                   network['assigned_networks'])
                            for networks_plane in merged_assigned_networks:
                                network_plane_names = networks_plane['name'].split(',')
                                network_plane_ip = networks_plane.get('ip')
                                if network_plane_ip:
                                    occupied_network_ips.append(network_plane_ip)
                                    check_ip_exist_and_in_cidr_range(values['cluster'],
                                                                     network_plane_names[0],
                                                                     network_plane_ip,
                                                                     occupied_network_ips, 
                                                                     session)
                                else:
                                    network_plane_ip = according_to_cidr_distribution_ip(values['cluster'],                                                     network_plane_names[0],
                                                                                         session)

                                if 'MANAGEMENT' in network_plane_names:
                                    change_host_name(values, network_plane_ip)
                                add_assigned_networks_data(context,network,values['cluster'],host_interface_ref,network_plane_names,network_plane_ip,session)
    ...
    try:
        ...
        for interface in host_interface:
            ...
            for assignnetwork in assignnetwork_list:
                query_network = session.query(models.Network).filter_by(id=assignnetwork.network_id).filter_by(deleted=False).first()
                if query_network:
                    assigned_networks_info = {'name':query_network.name, 
                                              'ip':assignnetwork.ip}

                    assigned_networks_list.append(assigned_networks_info)
                    if query_network.network_type in ['VXLAN','PRIVATE']:
...
def get_assigned_network(context, interface_id, network_id, session=None, force_show_deleted=False):
    session = session or get_session()
    try:
        query = session.query(models.AssignedNetworks).filter_by(interface_id=interface_id).filter_by(network_id=network_id).filter_by(deleted=False)
        host_assigned_network = query.one()
    except sa_orm.exc.NoResultFound:
        msg = "No assigned_network found with interface %s and network %s" % (interface_id, network_id)
        LOG.debug(msg)
        raise exception.NotFound(msg)
...
def _discover_host_update(context, values, discover_host_id):
    ...
    :param context: Request context
    :param values: A dict of attributes to set
    :param discover_host_id: If None, create the discover host, otherwise, find and update it
    """
    # NOTE(jbresnah) values is altered in this so a copy is needed
    values = values.copy()
    session = get_session()
    with session.begin():
        if discover_host_id:
            discover_host_ref = _discover_host_get(context, discover_host_id, session=session)
...
def _discover_host_get(context, discover_host_id, session=None, force_show_deleted=False):
    """Get an host or raise if it does not exist."""
...
def discover_host_get(context, discover_host_id, session=None, force_show_deleted=False):
    discover_host = _discover_host_get(context, discover_host_id, session=session,
                       force_show_deleted=force_show_deleted)
    return discover_host
...
def discover_host_get_all(context, filters=None, marker=None, limit=None,
    ...
    if marker is not None:
        marker_discover_host = _discover_host_get(context,
                                  marker,
    ...
    query = session.query(models.DiscoverHost).filter_by(deleted=showing_deleted)
...
def get_discover_host_detail(context, discover_host_id, session=None, force_show_deleted=False):
    '''
    '''
    session = session or get_session()
    try:
        query = session.query(models.DiscoverHost).filter_by(id=discover_host_id, deleted=False)
        # filter out deleted images if context disallows it
        if not force_show_deleted and not context.can_see_deleted:
            query = query.filter_by(deleted=False)
        discover_host = query.one()
                ...
                if network_cidr and network_cidr != values['cidr']:
                    if values['cidr'] != network_cidr:
                        ...
                        for tmp_ip in query_ip_list:
                            ip_pop=tmp_ip.values().pop()
                            if ip_pop:
                                if is_in_cidr_range(ip_pop, network_cidr):
                                    msg = "Error:Distribution ip by CIDR is being used, and the CIDR is not allowed to change."
                                    LOG.error(msg)
                                    raise exception.Forbidden(msg)
