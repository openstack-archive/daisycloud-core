...
class InstallTask(object):
    ...
    def _run(self):
        ...
        recycle_num_of_hosts_with_role = (len(hosts_with_role_need_os) +
        ...
        while order_hosts_need_os:
            ...
            if run_once_flag and recycle_number == recycle_num_of_hosts_with_role:
                if role_hosts_need_os:
                    ...
                    if install_backends_flag:
                        ...
                        LOG.info(_("hosts of controller role install "
                            ...
                            "applications for cluster %s." % self.cluster_id))
                        self._backends_install()
                    else:
                        ...
                        host_status = {'messages': 'host of controller role '
                                    'install failed,stop installing backend'}
                        run_once_flag = False
...
class Controller(controller.BaseController):
    ...
    def install_cluster(self, req, install_meta):
        ...
        if not daisy_cmn.in_cluster_list(cluster_id):
            LOG.info(_("daisy_cmn.cluster_install_entry_list append %s" % cluster_id))
    ...
    def _get_all_tecs_hosts(self, req, cluster_id, exclusive_state=[]):
        hosts_id = set()
        ...
        for role in roles:
            ...
            for role_host in role_hosts:
                if role_host['status'] not in exclusive_state:
                    hosts_id.add(role_host['host_id'])
        return hosts_id

    def _check_uninstall_hosts(self, req, cluster_id, uninstall_hosts):
        for host_id in uninstall_hosts:
            host_detail = registry.get_host_metadata(req.context, host_id)
            computer_role = ['COMPUTER']
            if host_detail.get('role'):
                if set(host_detail['role']) != set(computer_role):
                    exclusive_state = [tecs_state['INIT'],
                                       tecs_state['UNINSTALLING']]
                    all_tecs_hosts = self._get_all_tecs_hosts(
                        req, cluster_id, exclusive_state)
                    # 1. if assigned hosts include host which is 'init',
                    # it may be more than hosts from _get_all_tecs_hosts()
                    # who exclusive 'init'.
                    # 2. when all hosts is 'init', any host can be
                    # assigned to uninstall
                    if set(uninstall_hosts) >= all_tecs_hosts:
                        return
                    msg = "Assigned nodes invalid, you should select as: "\
                          "1. All nodes are 'COMPUTER'; 2. if uninstalling "\
                          "CONTROLLER nodes, you must select all nodes which "\
                          "haven't been uninstalled"
                    LOG.error(msg)
                    raise HTTPBadRequest(explanation=msg, request=req)
            else:
                 msg = "No role for host %s, so it can't be uninstalled" % host_id
                 LOG.error(msg)
                 raise HTTPBadRequest(explanation=msg, request=req)
    @utils.mutating
    def uninstall_cluster(self, req, cluster_id, install_meta):
        ...
        for backend in backends:
            if backend == daisy_cmn.tecs_backend_name:
                self._check_uninstall_hosts(req,
                                            cluster_id,
                                            uninstall_hosts)
            backend_driver = driver.load_deployment_dirver(backend)
