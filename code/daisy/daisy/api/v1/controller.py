...
class BaseController(object):
    ...
    def get_template_func_meta_or_404(self, request, template_func_id,
        ...
        context = request.context
        try:
            return registry.get_template_func_metadata(context, template_func_id,
                                                       **params)
        except exception.NotFound:
    ...
    def get_template_config_meta_or_404(self, request, template_config_id):
        """
        Grabs the template_config metadata for an template_config with a supplied
        identifier or raises an HTTPNotFound (404) response
        ...
        context = request.context
        try:
            return registry.get_template_config_metadata(context, template_config_id)
        except exception.NotFound:
            msg = "template_config with identifier %s not found" % template_config_id
            LOG.debug(msg)
            raise webob.exc.HTTPNotFound(
                msg, request=request, content_type='text/plain')
        except exception.Forbidden:
    ...
    def get_template_service_meta_or_404(self, request, template_service_id):
        """
        Grabs the template_service metadata for an template_service with a supplied
        identifier or raises an HTTPNotFound (404) response
        ...
        context = request.context
        try:
            return registry.get_template_service_metadata(context, template_service_id)
        except exception.NotFound:
            msg = "template_service with identifier %s not found" % template_service_id
            LOG.debug(msg)
            raise webob.exc.HTTPNotFound(
                msg, request=request, content_type='text/plain')
        except exception.Forbidden:
