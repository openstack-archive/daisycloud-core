...
tecs_install_path = tecs_cmn.tecs_install_path
...
def _get_host_private_networks(host_detail, cluster_private_networks_name):
    """
    User member nic pci segment replace the bond pci,
    we use it generate the mappings.json.
...
def _write_private_network_cfg_to_json(req, cluster_id, private_networks):
    """
    Generate cluster private network json.
    We use the json file after tecs is installed.
        ...
        for private_network in private_networks[k]:
            ...
            for assign_network in assign_networks:
                ...
                if not vswitch_type or not physnet_name:
                    LOG.error(
                        "private networks vswitch_type or\
                             physnet name is invalid!")
                    continue
                ...
                if 0 == cmp("bond", type):
                    if not pci1 or not pci2 or not\
                            slave1 or not slave2 or not mode:
                        LOG.error(
                            "when type is 'bond',\
                            input params is invalid in private networks!")
                        continue
...
def _conf_private_network(req, cluster_id, host_private_networks_dict,
    ...
    for k in host_private_networks_dict.keys():
        ...
        for host_private_network in host_private_networks:
            assigned_networks = host_private_network.get(
            ...
            private_network_info = \
                [network for assigned_network in assigned_networks
                 ...
                 if assigned_network and assigned_network[
                     'name'] == network['name']]

            host_private_network['assigned_networks'] = private_network_info
            config_neutron_ml2_vlan_ranges += \
                ["%(name)s:%(vlan_start)s:%(vlan_end)s" %
                 {'name': network['name'], 'vlan_start':network[
                     'vlan_start'], 'vlan_end':network['vlan_end']}
                 for network in private_network_info
                 if network['name'] and network['vlan_start'] and
                 network['vlan_end']]

    physic_network_cfg = {}
    if _write_private_network_cfg_to_json(
       req, cluster_id, host_private_networks_dict):
        physic_network_cfg['json_path'] = daisy_tecs_path + \
            "/" + cluster_id + "/" + "mappings.json"
    if config_neutron_ml2_vlan_ranges:
        host_private_networks_vlan_range = ",".join(
            list(set(config_neutron_ml2_vlan_ranges)))
...
def _get_bridge_mappings(interface):
    try:
        network = interface['assigned_networks'][0]
    except:
        return {}

    bridge_mappings = {}
    if network.get('network_type') in ['DATAPLANE']:
        nic = interface.get('name')
        phynet_name = network.get('name')
...
def get_network_config_for_dvs(host_private_networks_dict,
    ...
    host_private_networks_dict_for_dvs =\
        copy.deepcopy(host_private_networks_dict)
    ...
    for host_private_network in host_private_networks_dict_for_dvs:
        ...
        private_network_info = \
            [network for private_network in private_networks
             for network in cluster_private_network_dict
                if private_network and private_network[
                    'name'] == network['name']]
        host_private_network['assigned_networks'] = private_network_info
    return host_private_networks_dict_for_dvs


def get_dvs_cpu_sets(req, cluster_id, role_hosts):
    """
    dvs_cpu_list = [{'IP': 'ip1', 'DVS_CPU': [1,2,3,4]},
                    {'IP': 'ip2', 'DVS_CPU': [2,3,4,5]}]
    """
    dvs_cpu_list = []
    cluster_networks =\
    ...
    for role_host in role_hosts:
        ...
        if dvs_interfaces and 'dvs_cpus' in host_detail:
            management_ip = tecs_cmn.get_host_network_ip(req,
                                                         host_detail,
                                                         cluster_networks,
                                                         'MANAGEMENT')
            dvs_cpu_dict = {}
            dvs_cpu_dict['IP'] = management_ip
            dvs_cpu_dict['DVS_CPU'] =\
                utils.cpu_str_to_list(host_detail['dvs_cpus'])
            dvs_cpu_list.append(dvs_cpu_dict)
    return dvs_cpu_list
...
def _get_interfaces_network(req, host_detail, cluster_networks):
    has_interfaces = {}
    host_mngt_network = tecs_cmn.get_host_interface_by_network(
        host_detail, 'MANAGEMENT')
    host_mgnt_ip = tecs_cmn.get_host_network_ip(
        req, host_detail, cluster_networks, 'MANAGEMENT')
    host_mgnt_netmask = tecs_cmn.get_network_netmask(
    ...
    # host_deploy_network = tecs_cmn.get_host_interface_by_network(
    #     host_detail, 'DEPLOYMENT')
    host_deploy_network_info = tecs_cmn.get_host_interface_by_network(
        host_detail, 'DEPLOYMENT')
    # note:"is_deployment" can't label delpoyment network, it only used to
    # label dhcp mac
    if host_deploy_network_info:
        host_deploy_ip = tecs_cmn.get_host_network_ip(
            req, host_detail, cluster_networks, 'DEPLOYMENT')
        host_deploy_netmask = tecs_cmn.get_network_netmask(
            cluster_networks, 'DEPLOYMENT')
    ...
    mngt_network_cidr = tecs_cmn.get_network_cidr(
        cluster_networks, 'MANAGEMENT')
    host_storage_networks = [network for network in cluster_networks
                             if network['network_type'] == 'STORAGE' and
                             network['cidr'] != mngt_network_cidr]
    if host_storage_networks:
        host_storage_network_info = tecs_cmn.get_host_interface_by_network(
            host_detail, host_storage_networks[0]['name'])
        if host_storage_network_info:
            host_storage_ip = tecs_cmn.get_host_network_ip(
                req, host_detail, cluster_networks,
                host_storage_networks[0]['name'])
            host_storage_netmask = tecs_cmn.get_network_netmask(
                cluster_networks, host_storage_networks[0]['name'])
    ...
    host_public_network_info = tecs_cmn.get_host_interface_by_network(
        host_detail, 'PUBLICAPI')
    if host_public_network_info:
        public_vlan_id = tecs_cmn.get_network_vlan_id(
            cluster_networks, 'PUBLICAPI')
        if public_vlan_id:
            public_nic_name = host_public_network_info[
                'name'] + '.' + public_vlan_id
        else:
            public_nic_name = host_public_network_info['name']

        host_public_ip = tecs_cmn.get_host_network_ip(
            req, host_detail, cluster_networks, 'PUBLICAPI')
        host_public_netmask = tecs_cmn.get_network_netmask(
            cluster_networks, 'PUBLICAPI')
        ...
        has_interfaces['publicapi'] = host_public_network_info

    cluster_heartbeat_network_dict = [
        network for network in cluster_networks if network[
            'network_type'] == 'HEARTBEAT']
    cluster_heartbeat_networks_name = [
        network['name'] for network in cluster_heartbeat_network_dict]
    for heartbeat_network_name in cluster_heartbeat_networks_name:
        host_heartbeat_network_info = tecs_cmn.get_host_interface_by_network(
            host_detail, heartbeat_network_name)
        if not host_heartbeat_network_info:
            cluster_heartbeat_networks_name.remove(heartbeat_network_name)
    if len(cluster_heartbeat_networks_name) >= 1:
        host_heartbeat1_network_info = tecs_cmn.get_host_interface_by_network(
            host_detail, cluster_heartbeat_networks_name[0])
        host_heartbeat1_network_info['ip'] = tecs_cmn.get_host_network_ip(
            req, host_detail, cluster_networks,
            cluster_heartbeat_networks_name[0])
        has_interfaces['heartbeat1'] = host_heartbeat1_network_info
    if len(cluster_heartbeat_networks_name) == 2:
        host_heartbeat2_network_info = tecs_cmn.get_host_interface_by_network(
            host_detail, cluster_heartbeat_networks_name[1])
        host_heartbeat2_network_info['ip'] = tecs_cmn.get_host_network_ip(
            req, host_detail, cluster_networks,
            cluster_heartbeat_networks_name[1])
        has_interfaces['heartbeat2'] = host_heartbeat2_network_info
    return has_interfaces


def _get_host_nic_name(cluster_network, host_detail):
    """
    Different networking will generate different ha port name,
    ...
    mgr_interface_info = tecs_cmn.get_host_interface_by_network(
        copy_host_detail, 'MANAGEMENT')
    nic_info = [network
                for network in cluster_network
                for netname in mgr_interface_info.get(
                    'assigned_networks', None)
                if network.get('name', None) == netname]

    nic_capability = [info['capability']
                      for info in nic_info if info[
                      'network_type'] != "DATAPLANE"]
    if not nic_capability or nic_capability == [None]:
        return mgr_interface_info['name']

    mgr_nic_info = [mgr_net for mgr_net in nic_info if mgr_net[
        'network_type'] == "MANAGEMENT"][0]
    # if private and management plane is unifier
    if set(["DATAPLANE", "MANAGEMENT"]).issubset(set([info[
            'network_type'] for info in nic_info])):
        ...
        # if ip at outer
        if mgr_interface_info.get('ip', None) and mgr_interface_info.get(
                'name', None):
    ...
    # if ip at outer
    if mgr_interface_info.get('ip', None) and\
            mgr_interface_info.get('name', None):
        return "v_" + mgr_interface_info['name']
...
def _get_vxlan_vni_range(network):
    if network.get('vni_start') and network.get('vni_end'):
        return '%s:%s' % (network['vni_start'], network['vni_end'])
    else:
        return '1000:3000'


def get_share_cluster_disk_services(req, role_id):
    service_disks = tecs_cmn.get_service_disk_list(req, {'role_id': role_id})
    share_cluster_disk_services = []

    for service_disk in service_disks:
        if service_disk['disk_location'] == 'share_cluster':
            share_cluster_disk_services.append(service_disk['service'])
    return share_cluster_disk_services
...
def get_cluster_tecs_config(req, cluster_id):
    ...
    segment_type = {}
    dataplane_network_dict = [network for network in cluster_networks if
                              network['network_type'] == 'DATAPLANE']
    if dataplane_network_dict[0]['segmentation_type'] in ['vlan']:
        cluster_private_network_dict = dataplane_network_dict
        cluster_vxlan_network_dict = []
        segment_type.update({'vlan': ''})
    elif dataplane_network_dict[0]['segmentation_type'] in ['vxlan']:
        cluster_private_network_dict = []
        cluster_vxlan_network_dict = dataplane_network_dict
        segment_type.update({'vxlan': {'vni_range': _get_vxlan_vni_range(
            dataplane_network_dict[0])}})
    else:
        cluster_private_network_dict = []
        cluster_vxlan_network_dict = []

    cluster_private_networks_name = [network['name'] for network in
                                     cluster_private_network_dict]
    cluster_vxlan_networks_name = [network['name'] for network in
                                   cluster_vxlan_network_dict]
    ...
    for role in roles:
        ...
        for role_host in role_hosts:
            ...
            sorted_host_detail = tecs_cmn.sort_interfaces_by_pci(
                cluster_networks, host_detail)
            host_private_networks_list =\
                _get_host_private_networks(sorted_host_detail,
                                           cluster_private_networks_name)
            # get ha nic port name
            if role['name'] == "CONTROLLER_HA":
                mgr_nic_name = _get_host_nic_name(
            ...
            has_interfaces = _get_interfaces_network(
                req, host_detail, cluster_networks)
            ...
            host_mgnt = host_detail['name'] if cluster_data[
                'use_dns'] else host_mgnt_ip

            # host_mgnt_ip used to label who the private networks is
            host_private_networks_dict[host_mgnt] = host_private_networks_list
            if role['name'] == 'COMPUTER':
                host_vxlan_network_list = _get_host_private_networks(
                    sorted_host_detail, cluster_vxlan_networks_name)
                if host_vxlan_network_list:
                    host_private_networks_dict_for_dvs = {}
                    host_vxlan_network_dict[host_mgnt] = \
                        get_network_config_for_dvs(
                        host_vxlan_network_list, cluster_vxlan_network_dict)
                elif host_private_networks_list:
                    host_vxlan_network_dict = {}
                    host_private_networks_dict_for_dvs[host_mgnt] = \
                        get_network_config_for_dvs(
                        host_private_networks_list,
                        cluster_private_network_dict)
        ...
        if role['name'] == 'COMPUTER':
            dvs_cpu_set = get_dvs_cpu_sets(req, cluster_id, role_hosts)
            if dvs_cpu_set:
                dvs_cpu_sets.extend(dvs_cpu_set)
    other_config.update({'tecs_installed_hosts': tecs_installed_hosts})
    # replace private network
    physic_network_cfg = _conf_private_network(
        req, cluster_id, host_private_networks_dict,
        cluster_private_network_dict)
    dvs_cfg = conf_dvs(
        req, host_vxlan_network_dict, host_private_networks_dict_for_dvs)
    # set for dvs_cpu_sets
    dvs_cfg['dvs_cpu_sets'] = dvs_cpu_sets

    other_config.update({'physic_network_config': physic_network_cfg})
    other_config.update({'dvs_config': dvs_cfg})
    other_config.update({'segmentation_type': segment_type})
...
def get_host_name_and_mgnt_ip(tecs_config):
    ...
    ip_list = []
    ha_nodes_ip = set()
    nodes_ips = {'ha': set(), 'lb': set(), 'computer': set()}
    ...
    for role_name, role_configs in tecs_config.items():
        ...
        for host in role_configs['host_interfaces']:
            ...
            if role_name == 'CONTROLLER_HA':
                ha_nodes_ip.add(host_mgt['ip'])
            if host_mgt['ip'] in tecs_config['OTHER']['tecs_installed_hosts'] \
                    or host['name'] \
                    in tecs_config['OTHER']['tecs_installed_hosts']:
                continue
            if role_name == 'CONTROLLER_HA':
                nodes_ips['ha'].add(host_mgt['ip'])
            if role_name == 'CONTROLLER_LB':
                nodes_ips['lb'].add(host_mgt['ip'])
            if role_name == 'COMPUTER':
                nodes_ips['computer'].add(host_mgt['ip'])
    return name_ip_list, nodes_ips, ha_nodes_ip
...
def config_dnsmasq_server(host_ip_list, domain_ip_list, password='ossdbg1'):
    ...
    for host_ip in host_ip_list:
        ...
        config_scripts = [
            "sed -i '/^[^#]/s/no-resolv[[:space:]]*/\#no-resolv/'\
                                                            %s" % dns_conf,
        ...
        tecs_cmn.run_scrip(config_scripts, host_ip, password,
                           msg='Failed to config cache of dns server on %s' %
                               host_ip)
        ...
        tecs_cmn.run_scrip(config_ip_scripts, host_ip, password,
                           'Failed to config domain-ip of dns server on %s' %
                           host_ip)

        service_start_scripts = [
            "dns_linenumber=`grep -n \"^[\
            [:space:]]*ExecStart=/usr/sbin/dnsmasq -k\" "
            "/usr/lib/systemd/system/dnsmasq.service|cut -d \":\" -f 1` && "
            "sed -i \"${dns_linenumber}c ExecStart=/usr/sbin/dnsmasq -k "
            "--dns-forward-max=150\" /usr/lib/systemd/system/dnsmasq.service",
            "for i in `ps -elf | grep dnsmasq |grep -v grep | awk -F ' ' '{\
                print $4}'`;do kill -9 $i;done ",
            "systemctl daemon-reload && systemctl enable dnsmasq.service && "
            "systemctl restart dnsmasq.service"]
        tecs_cmn.run_scrip(service_start_scripts, host_ip, password,
                           msg='Failed to start service of dns server on %s' %
                               host_ip)


def config_dnsmasq_client(host_ip_list, ha_ip_list, password='ossdbg1'):
    dns_client_file = "/etc/resolv.conf"
    tmp_dns_client_file = "/etc/resolv.conf.tmp"
    config_scripts = []
    for ha_ip in ha_ip_list:
        config_scripts.append("echo 'nameserver %s' >> %s" %
                              (ha_ip, tmp_dns_client_file))
    config_scripts.append('cat %s > %s' % (tmp_dns_client_file,
                                           dns_client_file))
    config_scripts.append('rm -rf %s' % tmp_dns_client_file)
    for host_ip in host_ip_list:
        if host_ip not in ha_ip_list:
            tecs_cmn.run_scrip(config_scripts, host_ip, password,
                               msg='Failed to config dns client on %s' %
                                   host_ip)

    tecs_cmn.run_scrip(config_scripts,
                       msg='Failed to config dns client on daisy host')


def config_nodes_hosts(host_ip_list, domain_ip, password='ossdbg1'):
    hosts_file = "/etc/hosts"
    tmp_hosts_file = "/etc/hosts.tmp"
    config_scripts = ['cat /etc/hosts > %s' % tmp_hosts_file]
    for name_ip in domain_ip:
        config_scripts.append("linenumber=`grep -n '%s$' %s | "
                              ...
                              (name_ip.keys()[0],
                               tmp_hosts_file, tmp_hosts_file))
        config_scripts.append("echo '%s %s' >> %s" % (name_ip.values()[0],
                                                      name_ip.keys()[0],
                                                      tmp_hosts_file))
    config_scripts.append('cat %s > %s' % (tmp_hosts_file, hosts_file))
    config_scripts.append('rm -rf %s' % tmp_hosts_file)
    for host_ip in host_ip_list:
        tecs_cmn.run_scrip(config_scripts, host_ip, password,
                           msg='Failed to config /etc/hosts on %s' % host_ip)
    tecs_cmn.run_scrip(config_scripts,
                       msg='Failed to config /etc/hosts on daisy host')
...
def revise_nova_config(computer_nodes, ha_vip, public_vip, compute_ip_domain,
    ...
    for host_ip in computer_nodes:
        ...
        tecs_cmn.run_scrip(scripts, host_ip, password,
                           msg='Failed to config nova on %s' % host_ip)
    ...
    for host_ip in ha_nodes:
        config_scripts = ["linenumber1=`grep -n 'ServerAlias %s' "
                          ...
                          "${linenumber1}d %s" % (host_ip,
                                                  dashboard_file,
                                                  dashboard_file),
                          "linenumber2=`grep -n 'ServerAlias \
                                                        %s' %s| awk -F ':' '"
                          "{print $1}'` && [ ! -z ${linenumber2} ] && sed -i "
                          "${linenumber2}d %s" % (ha_vip, dashboard_file,
                                                  dashboard_file),
                          "linenumber3=`grep -n 'ServerAlias \
                                                    %s' %s| awk -F ':' '"
                          "{print $1}'` && [ ! -z ${linenumber3} ] && sed -i "
                          "${linenumber3}d %s" % (public_vip, dashboard_file,
                                                  dashboard_file),
                          'dasboard_linenumber1=`grep \
                          -n "ServerAlias localhost" '
                          '%s|cut -d ":" -f 1` && sed -i \
                            "${dasboard_linenumber1}a '
                          'ServerAlias %s" %s' % (dashboard_file, host_ip,
                                                  dashboard_file),
                          'dasboard_linenumber1=`grep -n \
                            "ServerAlias localhost" %s'
                          '|cut -d ":" -f 1` && sed -i \
                                        "${dasboard_linenumber1}a '
        ...
        tecs_cmn.run_scrip(config_scripts, host_ip, password,
                           msg='Failed to config horizon on %s' % host_ip)
    restart_http_scripts = ['systemctl daemon-reload &&'
                            'systemctl restart httpd.service']
    try:
        subprocess.check_output(restart_http_scripts, shell=True,
...
class TECSInstallTask(Thread):
        ...
        with open(self.log_file, "r") as tmp_fp:
            ...
            for lnum, lcontent in enumerate(tmp_fp, 1):
                ...
                if -1 != lcontent.find("successfully"):
                    ...
                    self.message = \
                        "Tecs install error, see line %s in '%s'" % (
        ...
        for role in roles:
            ...
            for role_host in role_hosts:
                if role_host['status'] != tecs_state['ACTIVE']:
                    ...
                    daisy_cmn.update_role_host(
                        self.req, role_host['id'], role_host)
    ...
    def _generate_tecs_config_file(self, cluster_id, tecs_config):
        ...
        if tecs_config:
            ...
            tecs_cmn.mkdir_tecs_install()
            cp_ha_conf = "\cp %s /home/tecs_install/" % ha_config_file
            tecs_conf = "\cp %s /home/tecs_install/" % tecs_config_file
            daisy_cmn.subprocess_call(cp_ha_conf)
            daisy_cmn.subprocess_call(tecs_conf)
    ...
    def run(self):
        try:
            ...
            LOG.info(_("TECS version package installed failed for"
                       " cluster %s." % self.cluster_id))
            LOG.exception(e.message)
        else:
            if not self.need_install:
                return
            LOG.info(_("TECS version package installed completely for"
                       " cluster %s." % self.cluster_id))

            LOG.info("Config provider ...")
            tecs_cmn.inform_provider_cloud_state(
                self.req.context, self.cluster_id, operation='add')
            time_cost = str(round((time.time() - start_time) / 60, 2))
            LOG.info(
                _("It totally takes %s min for installing tecs" % time_cost))
            ...
            LOG.info("Push configs for installing hosts ...")
            config_backend_name = 'clushshell'
            backend_driver = manager.configBackend(config_backend_name,
                                                   self.req)

            params = {'filters': {'cluster_id': self.cluster_id}}
            nodes = registry.get_hosts_detail(self.req.context, **params)
            push_nodes_id = [node['id'] for node in nodes
                             if node['status'] == 'with-role' and
                             node['role_status'] == tecs_state['INSTALLING']]
            components_name = ['nova']
            backend_driver.push_config_by_hosts(push_nodes_id,
                                                components_name)

            LOG.info("Config neutron ...")
            result = config.get_conf(
                self.tecs_config_file,
            ...
            LOG.info(_("TECS installed successfully for cluster %s."
                       % self.cluster_id))
        finally:
            if not self.need_install:
                return
    ...
    def _run(self):
        ...
        def executor(**params):
            ...
            if os.path.exists(self.log_file):
                params['tell_pos'] = self._check_install_log(
                    params.get('tell_pos', 0))
            ...
            if progress == 100:
                self.progress = 100
            elif self.progress != progress and progress >= 3:
        ...
        tecs_config = get_cluster_tecs_config(self.req, self.cluster_id)
        name_ip_list, self.nodes_ips, ha_nodes_ip =\
            get_host_name_and_mgnt_ip(tecs_config)

        self.mgnt_ip_list = (self.nodes_ips['ha'] |
                             self.nodes_ips['lb'] |
        ...
        unreached_hosts = daisy_cmn.check_ping_hosts(self.mgnt_ip_list,
                                                     self.ping_times)
        if unreached_hosts:
            self.message =\
                "ping hosts %s failed" % ','.join(unreached_hosts)
            raise exception.InstallException(self.message)
        else:
            # os maybe not reboot completely, wait for 10s.
            time.sleep(10)
        ...
        LOG.info(_("Begin to trust me for all nodes with role..."))
        root_passwd = 'ossdbg1'

        daisy_cmn.trust_me(self.mgnt_ip_list, root_passwd)
        tecs_cmn.mkdir_tecs_install(self.mgnt_ip_list)

        if tecs_config['OTHER']['cluster_data']['use_dns']:
            self.dns_name_ip =\
                replace_ip_with_domain_name(self.req, tecs_config)
            ...
            config_dnsmasq_server(ha_nodes_ip, self.dns_name_ip)
            config_dnsmasq_client(self.mgnt_ip_list, ha_nodes_ip)
            config_nodes_hosts(self.mgnt_ip_list, self.dns_name_ip)
            host_domain = [name_ip.keys()[0] for name_ip in self.dns_name_ip
                           if name_ip.values()[0] in self.mgnt_ip_list]

            unreached_hosts = daisy_cmn.check_ping_hosts(host_domain,
                                                         self.ping_times)
            if unreached_hosts:
                self.message = "ping hosts %s failed after DNS\
                                configuration" % ','.join(unreached_hosts)
                raise exception.InstallException(self.message)
        else:
            config_nodes_hosts(self.mgnt_ip_list, name_ip_list)
        # generate tecs config must be after ping check
        self.tecs_config_file =\
            self._generate_tecs_config_file(self.cluster_id,
                                            tecs_config)

        # install network-configuration-1.1.1-15.x86_64.rpm
        if self.mgnt_ip_list:
            for mgnt_ip in self.mgnt_ip_list:
                LOG.info(_("Begin to install network-configuration\
                            on %s" % mgnt_ip))
                tecs_cmn.TecsShellExector(mgnt_ip, 'install_rpm')
        # network-configuration will restart network,
        # wait until ping test successfully
        time.sleep(10)
        unreached_hosts = daisy_cmn.check_ping_hosts(self.mgnt_ip_list,
                                                     self.ping_times)
        if unreached_hosts:
            self.message = "ping hosts %s failed after network\
                            configuration" % ','.join(unreached_hosts)
            raise exception.InstallException(self.message)

        (share_disk_info, volume_disk_info, share_cluster_disk_info) =\
            disk_array.get_disk_array_info(self.req, self.cluster_id)

        if volume_disk_info:
            LOG.info(_("Begin to config multipth on compute nodes..."))
            disk_array.config_compute_multipath(self.nodes_ips['computer'])

        if share_disk_info or share_cluster_disk_info:
            LOG.info(_("Begin to config multipth on HA nodes..."))
            disk_array.config_compute_multipath(self.nodes_ips['ha'])
            LOG.info(_("Begin to config Disk Array on ha nodes..."))
            array_nodes_addr =\
                tecs_cmn.get_disk_array_nodes_addr(self.req, self.cluster_id)
        ...
        tecs_version_pkg_file =\
            tecs_cmn.check_and_get_tecs_version(daisy_tecs_path)
        if not tecs_version_pkg_file:
            self.state = tecs_state['INSTALL_FAILED']
            self.message =\
                "TECS version file not found in %s" % daisy_tecs_path
            raise exception.NotFound(message=self.message)

        # use pattern 'tecs_%s_install' to distinguish
        # multi clusters installation
        ...
        # in case getting old log path
        self.progress_log_location =\
            "/var/tmp/packstack/%s" % self.cluster_id
        if os.path.exists(self.progress_log_location):
            os.remove(self.progress_log_location)

        install_cmd = "sudo %s conf_file %s" % (tecs_version_pkg_file,
                                                self.tecs_config_file)
        LOG.info(_("Begin to install TECS in cluster %s." % self.cluster_id))
        subprocess.Popen(install_cmd,
                         shell=True,
                         stdout=self.install_log_fp,
                         stderr=self.install_log_fp)
        ...
        while True:
            ...
            if self.progress == 100:
                if volume_disk_info and self.nodes_ips['ha']:
            ...
            params = executor(
                ...
                if_progress_file_read=params.get(
                    "if_progress_file_read", False),
