...
def _os_thread_bin(req, host_ip, node_meta, update_file, vpatch_id, update_object):
    ...
    with open(var_log_path, "w+") as fp:
        ...
        try:
            exc_result = _exec_upgrade(host_ip, vpatch_id, update_file, update_object, fp)
        except subprocess.CalledProcessError as e:
            if e.returncode == 255 and "reboot" in e.output.strip():
                host_meta['os_progress'] = 100
                host_meta['os_status'] = host_os_status['ACTIVE']
                host_meta['messages'] = "OS upgrade successfully, reboot"
                if not vpatch_id:
                    host_meta['version_patch_id'] = ""
                daisy_cmn.update_db_host_status(req, host_id, host_meta)
                LOG.info(
                    _("OS upgrade for %s successfully,system reboot!" % host_ip))
...
def _cmp_os_version(new_os_file, old_os_version,
    ...
    if old_os_version:
        ...
        cmp_script = "tfg_showversion /var/lib/daisy/versionfile/os/%s" \
                     " %s" % (new_os_file, old_os_version)
        try:
