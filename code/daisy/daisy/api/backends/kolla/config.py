...
def add_role_to_inventory(config_data):
    host_sequence = 1
    for control_ip in config_data['control_lb_ip']:
        update_inventory_file('multinode', 'control', control_ip, host_sequence, 'ssh')
        host_sequence = host_sequence + 1

    host_sequence = 1
    for network_ip in config_data['network_ip']:
        update_inventory_file('multinode', 'network', network_ip, host_sequence, 'ssh')
        host_sequence = host_sequence + 1

    host_sequence = 1
    for compute_ip in config_data['compute_ip']:
        update_inventory_file('multinode', 'compute', compute_ip, host_sequence, 'ssh')
        host_sequence = host_sequence + 1

    host_sequence = 1
    for storage_ip in config_data['storage_ip']:
        update_inventory_file('multinode', 'storage', storage_ip, host_sequence, 'ssh')
        host_sequence = host_sequence + 1
