...
def update_inventory_file_of_ip_lines(file_path, filename, ovs_dpdk_pmd_coremask_list):
    fp = file('%s/kolla-ansible/ansible/inventory/%s' % (file_path, filename))
    lines = []
    for line in fp:
        for mgnt_ip in ovs_dpdk_pmd_coremask_dict.keys():
            if mgnt_ip in line:
                line = line.split('\n')[0] + ' %s' % ovs_dpdk_pmd_coremask_dict[mgnt_ip]  + '\n'
        lines.append(line)
    fp.close()
    s = ''.join(lines)
    fp = file('%s/kolla-ansible/ansible/inventory/%s' % (
...
def enable_openvswitch_dpdk(kolla_config, file_path):
    if kolla_config['enable_dvs']:
        ...
        isolcpus_list = kolla_config['ovs_dpdk_pmd_coremask_list'].values()
        if len(set(isolcpus_list)) == 1:
            ...
            update_inventory_file_of_ip_lines(file_path, multinode,
                                              kolla_config['ovs_dpdk_pmd_coremask_list'])
