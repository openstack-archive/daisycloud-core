        ...
        for host in hosts_list:
            ...
            with open(var_log_path, "w+") as fp:
                ...
                cmd = "scp -o ConnectTimeout=10 \
                       ...
                       root@%s:/home/kolla" % host_ip
                daisy_cmn.subprocess_call(cmd, fp)
                cmd = 'clush -S -b -w %s \
                       chmod u+x /home/kolla/prepare.sh' % (host_ip,)
                daisy_cmn.subprocess_call(cmd, fp)
                try:
                    exc_result = subprocess.check_output(
                        'clush -S -b -w %s  /home/kolla/prepare.sh'
                        % (host_ip,), shell=True, stderr=subprocess.STDOUT)
                ...
                try:
                    exc_result = subprocess.check_output(
                        '/home/kolla_install/kolla/tools/kolla-ansible \
                        prechecks', shell=True, stderr=subprocess.STDOUT)
                except subprocess.CalledProcessError as e:
                    update_progress_to_db(req, role_id_list,
                                          kolla_state['INSTALL_FAILED'])
                ...
                try:
                    exc_result = subprocess.check_output(
                        '/home/kolla_install/kolla/tools/kolla-ansible deploy -i \
                        /home/kolla_install/kolla/ansible/inventory/multinode',
