...
def get_cluster_kolla_config(req, cluster_id):
    ...
    for role in roles:
        if role['name'] == 'CONTROLLER_LB':
            ...
            if len(set(mgt_macname_list)) != 1 or len(set(pub_macname_list)) != 1:
                msg = (_("hosts interface name of public and management must be same!"))
                LOG.error(msg)
...
def generate_kolla_config_file(cluster_id, kolla_config):
    ...
    if kolla_config:
        ...
        config.add_role_to_inventory(kolla_config)
    ...
    def _run(self):
        ...
        for host in hosts_list:
            ...
            with open(var_log_path, "w+") as fp:
                ...
                try:
                    exc_result = subprocess.check_output('/home/kolla_install/kolla/tools/kolla-ansible prechecks', shell=True, stderr=subprocess.STDOUT)
                except subprocess.CalledProcessError as e:
                    update_progress_to_db(req, role_id_list, kolla_state['INSTALL_FAILED'])
                ...
                try:
                    exc_result = subprocess.check_output('/home/kolla_install/kolla/tools/kolla-ansible deploy -i /home/kolla_install/kolla/ansible/inventory/multinode', shell=True, stderr=subprocess.STDOUT)
                except subprocess.CalledProcessError as e:
                    update_progress_to_db(req, role_id_list, kolla_state['INSTALL_FAILED'])
