...
class DaisyException(Exception):
    """
    ...
    def __init__(self, message=None, *args, **kwargs):
        ...
        super(DaisyException, self).__init__(message)
...
class MissingCredentialError(DaisyException):
    message = _("Missing required credential: %(required)s")


class BadAuthStrategy(DaisyException):
    message = _("Incorrect auth strategy, expected \"%(expected)s\" but "
                "received \"%(received)s\"")


class NotFound(DaisyException):
    message = _("An object with the specified identifier was not found.")


class BadStoreUri(DaisyException):
    message = _("The Store URI was malformed.")


class Duplicate(DaisyException):
    message = _("An object with the same identifier already exists.")


class Conflict(DaisyException):
    message = _("An object with the same identifier is currently being "
                "operated on.")


class StorageQuotaFull(DaisyException):
    message = _("The size of the data %(image_size)s will exceed the limit. "
                "%(remaining)s bytes remaining.")


class AuthBadRequest(DaisyException):
    message = _("Connect error/bad request to Auth service at URL %(url)s.")


class AuthUrlNotFound(DaisyException):
    message = _("Auth service at URL %(url)s not found.")


class AuthorizationFailure(DaisyException):
    message = _("Authorization failed.")


class NotAuthenticated(DaisyException):
    message = _("You are not authenticated.")


class UploadException(DaisyException):
    message = _('Image upload problem: %s')


class Forbidden(DaisyException):
...
class AuthorizationRedirect(DaisyException):
    message = _("Redirecting to %(uri)s for authorization.")


class ClientConnectionError(DaisyException):
    message = _("There was an error connecting to a server")


class ClientConfigurationError(DaisyException):
    message = _("There was an error configuring the client.")


class MultipleChoices(DaisyException):
    message = _("The request returned a 302 Multiple Choices. This generally "
                "means that you have not included a version indicator in a "
                "request URI.\n\nThe body of response returned:\n%(body)s")


class LimitExceeded(DaisyException):
...
class ServiceUnavailable(DaisyException):
...
class ServerError(DaisyException):
    message = _("The request returned 500 Internal Server Error.")


class UnexpectedStatus(DaisyException):
    message = _("The request returned an unexpected status: %(status)s."
                "\n\nThe response body:\n%(body)s")


class InvalidContentType(DaisyException):
    message = _("Invalid content type %(content_type)s")


class BadRegistryConnectionConfiguration(DaisyException):
    message = _("Registry was not configured correctly on API server. "
                "Reason: %(reason)s")


class BadDriverConfiguration(DaisyException):
    message = _("Driver %(driver_name)s could not be configured correctly. "
                "Reason: %(reason)s")


class MaxRedirectsExceeded(DaisyException):
    message = _("Maximum redirects (%(redirects)s) was exceeded.")


class InvalidRedirect(DaisyException):
    message = _("Received invalid HTTP redirect.")


class NoServiceEndpoint(DaisyException):
    message = _("Response from Keystone does not contain a Glance endpoint.")


class RegionAmbiguity(DaisyException):
    message = _("Multiple 'image' service matches for region %(region)s. This "
                "generally means that a region is required and you have not "
                "supplied one.")


class WorkerCreationFailure(DaisyException):
    message = _("Server worker creation failed: %(reason)s.")


class SchemaLoadError(DaisyException):
    message = _("Unable to load schema: %(reason)s")


class InvalidObject(DaisyException):
    message = _("Provided object does not match schema "
                "'%(schema)s': %(reason)s")


class UnsupportedHeaderFeature(DaisyException):
    message = _("Provided header feature is unsupported: %(feature)s")


class InUseByStore(DaisyException):
    message = _("The image cannot be deleted because it is in use through "
                "the backend store outside of daisy.")


class ImageSizeLimitExceeded(DaisyException):
...
class ArtifactLoadError(DaisyException):
...
class JsonPatchException(DaisyException):
