...
def dvs_get_cpu_sets(dic_numas, nics_info, device_numa_node, cpu_num=6):
    ...
    if not dic_numas or not dic_numas['numa_node0']:
        ...
        status['rc'] = return_code
        status['msg'] = msg
        cpu_set = {'high': [-4], 'low': [-4], 'dvs': {'dvsc': [-4],
                   'dvsp': [-4], 'dvsv': [-4]}, 'numa_node': -4}
    ...
    if numa_node < 0:
        if numa_node == -100:
            ...
            return_code = 2
            cpu_set = {'high': [-2], 'low': [-2], 'dvs': {'dvsc': [-2],
                       'dvsp': [-2], 'dvsv': [-2]}, 'numa_node': -2}
        elif numa_node == -200:
            msg = "Get numa node failed for DVS"
            return_code = 3
            cpu_set = {'high': [-3], 'low': [-3], 'dvs': {'dvsc': [-3],
                       'dvsp': [-3], 'dvsv': [-3]}, 'numa_node': -3}
        else:
            msg = "Invalid numa node '%s' for DVS, maybe you "\
                "need to upgrade BIOS version" % numa_node
            return_code = 1
            cpu_set = {'high': [-1], 'low': [-1], 'dvs': {'dvsc': [-1],
                       'dvsp': [-1], 'dvsv': [-1]}, 'numa_node': -1}
        LOG.error(msg)
        status['rc'] = return_code
        status['msg'] = msg
        return (status, cpu_set)
    ...
    if numa_key not in dic_numas:
        ...
        status['rc'] = return_code
        status['msg'] = msg
        cpu_set = {'high': [-5], 'low': [-5], 'dvs': {'dvsc': [-5],
                   'dvsp': [-5], 'dvsv': [-5]}, 'numa_node': -5}
        return (status, cpu_set)
    ...
    if len(dic_numas[numa_key]) < (cpu_num + 1):
        ...
        status['rc'] = return_code
        status['msg'] = msg
        cpu_set = {'high': [-6], 'low': [-6], 'dvs': {'dvsc': [-6],
                   'dvsp': [-6], 'dvsv': [-6]}, 'numa_node': -6}
        return (status, cpu_set)
...
def get_dvs_cpusets(numa_cpus, host_detail, host_hw_info):
    ...
    if dvs_nics_name:
        ...
        dvs_cpusets = {'high': [-7], 'low': [-7], 'dvs': {'dvsc': [-7],
                       'dvsp': [-7], 'dvsv': [-7]}, 'numa_node': -7}
...
def allocate_cpus(host_detail):
    ...
    if dvs_cpusets.get('dvs', {}):
        ...
        host_cpu_sets['suggest_dvs_cpus'] =\
            utils.cpu_list_to_str(dvs_cpusets['dvs'].get('dvsv', [])
            + dvs_cpusets['dvs'].get('dvsc', [])
