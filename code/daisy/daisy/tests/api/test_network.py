...
class TestNetworkApi(test.TestCase):
    ...
    def test_update_network(self):
        self.controller.get_network_meta_or_404 = mock.Mock(return_value=network_list[0])
        ...
        req.context = RequestContext(is_admin=True,
                                     ...
                                     tenant='fake tenamet')
        network_id = "123"
        network_meta = {'network_type': 'DATAPLANE', 'cluster_id':'test', 'gateway': '192.168.1.1'}
        registry.update_network_metadata = mock.Mock(return_value=network_meta)
        update_network = self.controller.update_network(req, network_id, network_meta)
        self.assertEqual(network_meta['network_type'], update_network['network_meta']['network_type'])
    ...
    def test_add_network(self):
        ...
        req.context = RequestContext(is_admin=True,
                                     user='fake user',
                                     tenant='fake tenamet')
        network_meta = {'network_type': 'DATAPLANE', 'gateway': '192.168.1.1', 'name':'pysnet1'}
        ...
        self.assertEqual(network_meta['network_type'], add_network['network_meta']['network_type'])
    ...
    def test_update_another_network_with_same_custom_name(self,
        ...
        get_cluster_meta.return_value = cluster_meta
        get_networks_detail.return_value = networks_detail
        self.assertRaises(webob.exc.HTTPConflict, self.controller.update_network,
                          req, network_id, network_meta)

    @mock.patch('daisy.registry.client.v1.api.get_assigned_networks_data_by_network_id')
    @mock.patch('daisy.registry.client.v1.api.get_networks_detail')
    @mock.patch('daisy.registry.client.v1.api.get_cluster_metadata')
    @mock.patch('daisy.registry.client.v1.api.get_network_metadata')
    def test_update_network_with_forbidden_change_segment_type(self,
                                                      get_network_meta,
                                                      get_cluster_meta,
                                                      get_networks_detail,
        ...
        networks_detail = [{'alias': None,
                           ...
                           'id': 'cf531581-a283-41dd-9e4e-4b98454d54e7',
                           'ip': None,
                           'ip_ranges': [{'end': '1.2.3.6', 'start': '1.2.3.4'}],
                           'ml2_type': 'ovs',
                           'mt': 1500,
                           'name': 'physnet1',
                           'network_type': 'DATAPLANE',
                           ...
                           'vni_end': 20,
                           'vni_start': 1}]
        assigned_network = [{'deleted': False,
        ...
        get_cluster_meta.return_value = cluster_meta
        get_networks_detail.return_value = networks_detail
        get_assigned_networks_data_by_network_id.return_value = assigned_network
        self.assertRaises(webob.exc.HTTPForbidden, self.controller.update_network,
                          req, network_id, network_meta)
