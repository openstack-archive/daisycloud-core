...
class TestHostTemplate(test.TestCase):
    ...
    def test_template_to_host_with_not_found(self):
        req = webob.Request.blank('/')
        req.context = RequestContext(is_admin=True,
                                     user='fake user',
                                     tenant='fake tenant')
    ...
    def test_template_to_host(self):
        ...
        registry.get_clusters_detail = mock.Mock(
            return_value=[{
                "id": "93ca3165-1a82-4c4a-914f-65279827e46e",
                "name": "test"}])
        self.controller._judge_ssh_host = mock.Mock(return_value=False)
        registry.update_host_metadata = mock.Mock(return_value=host_detail)
