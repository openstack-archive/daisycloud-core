...
class TestOsInstall(test.TestCase):
    ...
    @mock.patch('daisy.api.backends.common.update_db_host_status')
    @mock.patch("daisy.api.backends.common.get_host_detail")
    def test_upgrade_os(self, mock_get_host_detail, mock_do_update_db_status):
        req = webob.Request.blank('/')
        req.context = RequestContext(is_admin=True, user='fake user',
                                     tenant='fake tenant')
        version_id = "1"
    ...
    def test_exec_upgrade_with_redhat7(self):
        host_ip = '127.0.0.1'
    ...
    def test_exec_upgrade_with_patch(self):
        host_ip = '127.0.0.1'
        vpatch_id = "1234"
        update_file = "test.tar.gz"
        update_object = "vplat"
        exec_result = 'OS upgraded successfully'
        daisy_cmn.subprocess_call = mock.Mock(return_value="")
        subprocess.check_output = \
            mock.Mock(return_value=exec_result)
        result = install._exec_upgrade(host_ip, vpatch_id, update_file,
                                       update_object, None)
