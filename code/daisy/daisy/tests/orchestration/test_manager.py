...
class TestManager(test.TestCase):
    def setUp(self):
        ...
        self.req.context = RequestContext(is_admin=True, user='fake user',
                                          tenant='fake tenant')
        self.daisy_client = mock.Mock(return_value=Client(version="1.0", endpoint="http://127.0.0.1:8080")) 
    ...
    def test_get_active_compute(self):
        def do_host_list(**kwargs):
            host1 = MockHostList(os_status="active",role_status='active', id='aaaa', role=['COMPUTER'], interfaces='')
            host2 = MockHostList(os_status="active",role_status='init', id='bbbb', role=['CONTROLLER_HA'], interfaces='')
            host_list = [host1, host2]
            for host in host_list:
                yield host
        cluster_id = "123"
        ...
        hosts = self.Orchestration.get_active_compute(cluster_id, self.daisy_client)
