...
class Controller(object):
    ...
    def neutron_backend_add(self, req, body):
        ...
        if id and not utils.is_uuid_like(id):
            msg = _LI("Rejecting neutron_backend creation request for invalid neutron_backend "
        ...
        try:
            neutron_backend_data = self.db_api.neutron_backend_add(req.context, neutron_backend_data)
            msg = (_LI("Successfully created neutron_backend %s") %
                   neutron_backend_data["id"])
            LOG.info(msg)
            if 'neutron_backend' not in neutron_backend_data:
                neutron_backend_data = dict(neutron_backend=neutron_backend_data)
    ...
    def neutron_backend_update(self, req, id, body):
        ...
        neutron_backend_data = body["neutron_backend"]
        if id and not utils.is_uuid_like(id):
            msg = _LI("Rejecting cluster neutron_backend creation request for invalid "
        ...
        try:
            neutron_backend_data = self.db_api.neutron_backend_update(req.context, id, neutron_backend_data)
            ...
            msg = (_("Failed to update neutron_backend metadata.Got error: %s") %
    ...
    def neutron_backend_delete(self, req, id):
        ...
        """
        if id and not utils.is_uuid_like(id):
            msg = _LI("Rejecting neutron_backend delete request for invalid neutron_backend "
                      "id '%(bad_id)s'") % {'bad_id': id}
            LOG.info(msg)
            msg = _("Invalid neutron_backend id format")
            return exc.HTTPBadRequest(explanation=msg)
        ...
        try:
            neutron_backend_data = self.db_api.neutron_backend_destroy(req.context, id)
    ...
    def _list_neutron_backends(self, context, filters, params):
        ...
        try:
            ...
            LOG.warn(_LW("Access denied to neutron_backend %(id)s but returning "
    ...
    def neutron_backend_detail(self, req, id):
        ...
        try:
            neutron_backend_data = self.db_api.neutron_backend_detail(req.context, id)
