...
class ComponentManager(base.ManagerWithFind):
    ...
    def _component_meta_from_headers(self, headers):
        meta = {'properties': {}}
    ...
    @staticmethod
    def _format_component_meta_for_user(meta):
        for key in ['size', 'min_ram', 'min_disk']:
            if key in meta:
                try:
                    meta[key] = int(meta[key]) if meta[key] else 0
    ...
    def get(self, component, **kwargs):
        """Get the metadata for a specific component.

        :param component: host object or id to look up
    ...
    def list(self, **kwargs):
        ...
        def paginate(qp, return_request_id=None):
            ...
            url = '/v1/components/detail?%s' % urlparse.urlencode(qp)
            components, resp = self._list(url, "components")
        ...
        while True:
            seen_last_page = 0
            filtered = 0
            for component in paginate(params, return_request_id):
            ...
            if seen_last_page + filtered == 0:
                # Note(kragniz): we didn't get any components in the last page
                return

            if absolute_limit is not None and seen >= absolute_limit:
                # Note(kragniz): reached the limit of components to return
                return

            if page_size and seen_last_page + filtered < page_size:
                # Note(kragniz): we've reached the last page of the components
                return

            # Note(kragniz): there are more components to come
            params['marker'] = last_component
            seen_last_page = 0
    ...
    def delete(self, component, **kwargs):
        """Delete an component."""
        url = "/v1/components/%s" % base.getid(component)
    ...
    def update(self, component, **kwargs):
        ...
        hdrs.update(self._component_meta_to_headers(fields))
