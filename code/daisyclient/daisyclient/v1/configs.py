...
class ConfigManager(base.ManagerWithFind):
    ...
    def _config_meta_from_headers(self, headers):
        meta = {'properties': {}}
    ...
    @staticmethod
    def _format_config_meta_for_user(meta):
    ...
    def get(self, config, **kwargs):
        """Get the metadata for a specific config.

        :param config: image object or id to look up
        :rtype: :class:`Config`
        """
    ...
    def list(self, **kwargs):
        ...
        :param marker: begin returning configs that appear later in the config
                       list than that represented by this config id
        :param filters: dict of direct comparison filters that mimics the
                        structure of an config object
        ...
        def paginate(qp, return_request_id=None):
            ...
            url = '/v1/configs/detail?%s' % urlparse.urlencode(qp)
            configs, resp = self._list(url, "configs")
        ...
        while True:
            seen_last_page = 0
            filtered = 0
            for config in paginate(params, return_request_id):
                last_config = config.id
