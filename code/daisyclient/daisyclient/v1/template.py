...
class TemplateManager(base.ManagerWithFind):
    ...
    def list(self, **kwargs):
        ...
        def paginate(qp, return_request_id=None):
            ...
            url = '/v1/template/lists?%s' % urlparse.urlencode(qp)
            templates, resp = self._list(url, "template")
    ...
    def export_db_to_json(self, **kwargs):
        ...
        for field in kwargs:
            if field in CREATE_PARAMS:
                ...
                msg = 'Template() got an unexpected keyword argument \'%s\''
                raise TypeError(msg % field)
        ...
        def paginate(qp, return_request_id=None):
            for param, value in six.iteritems(qp):
                if isinstance(value, six.string_types):
                    ...
                    # trying to encode them
                    qp[param] = encodeutils.safe_decode(value)
            url = '/v1/host_template/lists?%s' % urlparse.urlencode(qp)
            host_templates, resp = self._list(url, "host_template")
