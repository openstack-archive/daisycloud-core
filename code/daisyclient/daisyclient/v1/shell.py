...
@utils.arg('--interfaces', metavar='<type=ether,name=eth5,mac=4C:AC:0A:AA:9C:EF,ip=ip_addr,netmask=netmask,gateway=gateway,is_deployment=False,assigned_networks=networkname1:ip1_networkname2:ip2_networkname3:ip3,pci=pci,mode=mode,slaves=eth0_eth1,vswitch_type=ovs>',
           nargs='+',
           help='node network interface detail, ip must be given if assigned_networks is empty,\
...
def do_host_add(gc, args):
    ...
    if args.interfaces: 
        for interface in args.interfaces:
            interface_info = {"pci":"", "mode":"", "gateway":"", "type": "", "name": "", "mac": "", "ip": "", "netmask": "", "assigned_networks": "", "slaves":"", "is_deployment":"", "vswitch_type":""}
            for kv_str in interface.split(","):                                
                ...
                if k in interface_info:                                        
                    ...
                    if k == "assigned_networks":
                        networks_list_obj = interface_info['assigned_networks'].split("_")
                        networks_list=[]
...
def do_host_update(gc, args):
    ...
    if args.interfaces: 
        for interfaces in args.interfaces:
            interface_info = {"pci":"", "mode":"", "gateway":"", "type": "", "name": "", "mac": "", "ip": "", "netmask": "", "mode": "","assigned_networks": "", "slaves":"", "is_deployment":"", "vswitch_type":""}
...
def do_discover_host_list(gc, args):
    ...
    columns = ['Id', 'Ip','User', 'Passwd', 'Status', 'Message', 'Host_id']
...
def do_cluster_list(gc, args):
    ...
    columns = ['ID', 'Name', 'Description', 'Nodes', 'Networks', 'Auto_scale', 'Use_dns']
...
@utils.arg('--disk-location', metavar='<DISK_LOCATION>',
           help='where disks used by backends application from, default is "local". \
                 "local" means disks come from local host, "share" means disks come from share storage devices')
...
def do_role_list(gc, args):
    ...
    columns = ['ID', 'Name','Description','Status','Progress','Config_Set_ID','CLUSTER_ID','TYPE','VIP','Deployment_Backend']
...
def do_role_detail(gc, args):
    ...
    if filters:
        ...
        roles = gc.roles.list(**kwargs)
        columns = ['ID', 'Name','Description','Status','Progress','Config_Set_ID','CLUSTER_ID','TYPE','VIP']
        utils.print_list(roles, columns)
...
@utils.arg('--vlan-start', metavar='<VLAN_START>', 
           help='vlan start of network.it should be a integer in "1~4096", and it must be appeared with vlan end')
@utils.arg('--vlan-end', metavar='<VLAN_END>', 
           help='vlan end of network.it should be a integer in "1~4096", and it must be appeared with vlan start')             
...
@utils.arg('--ml2-type', metavar='<ML2_TYPE>' ,
           help='ml2 type:"ovs", "sriov(direct)", "sriov(macvtap)", "ovs,sriov(direct)" or "ovs,sriov(macvtap)".\
                 when network-type is PRIVATE, ml2-type must be given')
...
def do_network_list(gc, args):
    ...
    columns = ['ID', 'Name', 'Cluster_id', 'Description', 'Vlan_start','Vlan_end','Gateway','Cidr','Type', 'Ip_ranges']
...
def do_discover_host(gc, args):
    ...
    discover_host = gc.hosts.discover_host(**kwargs)
    _daisy_show(discover_host)

@utils.arg('service', metavar='<SERVICE>', 
            help='service name who will use disk storage, suport db, glance and dbbackup.')
...
@utils.arg('--size', metavar='<SIZE>', 
            help='unit is G, and default is -1, it means to use all of the disk.')   
...
def do_service_disk_add(dc, args):
    ...
    fields = dict(filter(lambda x: x[0] in SERVICE_DISK_PARAMS, fields.items()))
...
def do_service_disk_delete(dc, args):
    ...
    for service_disk_id in args.service_disks:
        ...
        try:
            ...
            if args.verbose:
                print('[Fail]')
            print('%s: Unable to delete service_disk %s' % (e, service_disk_id))
            
@utils.arg('id', metavar='<ID>', 
            help='ID of service_disk.')
@utils.arg('--service', metavar='<SERVICE>', 
            help='service name who will use Disk Array device, suport db, glance and dbbackup.')
...
@utils.arg('--size', metavar='<SIZE>', 
            help='unit is G, and default is -1, it means to use all of the disk.')     
@utils.arg('--lun', metavar='<LUN>', 
            help='mark which lun is used for Disk Array device,default is 0.')  
def do_service_disk_update(dc, args):
    ...
    id = fields.pop('id')
    service_disk = utils.find_resource(dc.disk_array, id)
...
def do_service_disk_detail(dc, args):
    ...
    if filters:
        ...
        service_disk_info = dc.disk_array.service_disk_detail(fields.pop('id'), **fields)
        _daisy_show(service_disk_info)
    else:
...
@utils.arg('disk_array', metavar='<management_ips=ip1_ip2,data_ips=ip3_ip4,'
           ...
           help='management_ips: management interfaces ip of Disk Array\
                ...
                separate by ",", \
                when using FUJISTU Disk Array, DATA_IPS cannot be empty;\
                ...
                and "FUJISTU_ETERNUS" according by Disk Array device type,\
                separate by "_";\
...
def do_cinder_volume_add(dc, args):
    ...
    fields = dict(filter(lambda x: x[1] is not None, vars(args).items()))
    # Filter out values we can't use
    CINDER_BACKEND_PARAMS = daisyclient.v1.disk_array.CREATE_CINDER_BACKEND_PARAMS
    fields = dict(filter(lambda x: x[0] in CINDER_BACKEND_PARAMS, fields.items()))
...
def do_cinder_volume_delete(dc, args):
    ...
    for cinder_volume_id in args.cinder_volumes:
        ...
        try:
            ...
            if args.verbose:
                print('[Fail]')
            print('%s: Unable to delete cinder volume %s' % (e, cinder_volume_id))

@utils.arg('id', metavar='<ID>', 
            help='ID of cinder_volume.')
@utils.arg('--management-ips', metavar='<MANAGEMENT_IPS>', 
            help='management interfaces ip of Disk Array device, separate by ","')
@utils.arg('--data-ips', metavar='<DATA_IPS>', 
            help='data interfaces ip of Disk Array device, separate by ",", \
                  when using FUJISTU Disk Array, DATA_IPS cannot be empty')
...
@utils.arg('--volume-driver', metavar='<VOLUME_DRIVER>', 
            help='supports "KS3200_FCSAN", "KS3200_IPSAN" and "FUJISTU_ETERNUS"\
...
def do_cinder_volume_update(dc, args):
    ...
    CINDER_VOLUME_PARAMS =\
        daisyclient.v1.disk_array.CREATE_CINDER_BACKEND_INTER_PARAMS
    fields = dict(filter(lambda x: x[0] in CINDER_VOLUME_PARAMS, fields.items()))
    
    if fields.has_key('management_ips'):
        fields['management_ips'] = ','.join(fields['management_ips'].split("_"))
    if fields.has_key('data_ips'):
        fields['data_ips'] = ','.join(fields['data_ips'].split("_"))
    if fields.has_key('pools'):
...
def do_cinder_volume_detail(dc, args):
    ...
    kwargs = {'filters': filters}
    if filters:
        cinder_volume_info = dc.disk_array.cinder_volume_detail(fields.pop('id'), **fields)
        _daisy_show(cinder_volume_info)
    else:
...
@utils.arg('--description', metavar = '<DESCRIPTION>',
           help = 'Description of the template.')
@utils.arg('--type', metavar = '<TYPE>', 
            help = 'Export backend database based on type,for example:tecs,zenic')
...
def do_import_json_to_template(dc, args):
    ...
    import_json_to_template = dc.template.import_json_to_template(**dict_params)
...
def do_host_template_list(dc, args):
    ...
    columns = ['name','description','os_version_file','role','interfaces']
