...
class HTTPSAdapter(adapters.HTTPAdapter):
    ...
    https pool by setting daisyclient's
    one.
    """
    ...
    def _create_daisy_httpsconnectionpool(self, url):
    ...
    def get_connection(self, url, proxies=None):
        try:
            ...
            # The scheme is most likely daisy+https but check anyway
            if not url.startswith('daisy+https://'):
                raise

            return self._create_daisy_httpsconnectionpool(url)
...
class HTTPSConnectionPool(connectionpool.HTTPSConnectionPool):
    ...
    returns an instances of daisyclient's VerifiedHTTPSConnection
